...
// Pin para sendor de luminosidad
#define ldr_dpin A0
...
// Pines para control de pantalla LCD SPI
...
const char ssid[] = "RAMIREZ G";  // TODO cambiar por el nombre de la red WiFi
// Contraseña de la red WiFi
const char pass[] = "Andres_5588";  // TODO cambiar por la contraseña de la red WiFi
...
const char MQTT_HOST[] = "98.93.237.88";  // TODO Reemplace ip.maquina.mqtt por la IP del bróker MQTT que usted desplegó. Ej: 192.168.0.1
...
bool checkMeasures(float t, float h, float l) {
  ...
  if (isnan(t) || isnan(h)) {
    ...
    Serial.println("Error obteniendo los datos del sensor LDH");
    return false;
  }
  return true;
}
...
void receivedCallback(char* topic, byte* payload, unsigned int length) {
  ...
  if (data.indexOf("ALERT") >= 0) {
    alert = data;
    alertTime = millis();  // iniciar temporizador correctamente
  }
}
...
void handleStatusLEDs() {
  ...
  // verificar si alerta sigue activa usando el mismo temporizador
  bool alertActive = (alert.length() != 0) && ((millis() - alertTime) < ALERT_DURATION * 1000);
  ...
  if (alertActive) {
    ...
    // parpadeo LED rojo sincronizado
